...
void CDROM::scheduleInterrupt(u32 cycles) {
    scheduler.scheduleEvent(cycles, [this]() {
        m_irqFlags |= m_ints.front();
        bus.triggerInterrupt(InterruptType::CDROM);
        m_ints.pop();
    });
}

void CDROM::scheduleCommandFinish(u32 cycles) {
    m_emulator->scheduler->scheduleEvent(cycles, EventType::CDROM, [this]() { tryFinishCommand(); });
}

void CDROM::scheduleStartCommand(u32 cycles) {
    m_emulator->scheduler->scheduleEvent(cycles, EventType::CDROM, [this]() { tryStartCommand(); });
}

void CDROM::scheduleRead() {
    u32 speed = m_mode.Speed ? 150 : 75;
    u32 cycles = frequency / speed;
    if (m_delayFirstRead) {
        cycles = 33868800 * 3;
        m_delayFirstRead = false;
    }
    m_emulator->scheduler->scheduleEvent(cycles, EventType::CDROM, [this]() { readSector(); });
}
...
void CDROM::tryStartCommand() {
    ...
    log<Log::CDROM>("Starting Command: {}\n", magic_enum::enum_name(m_command));
    ...
    if (m_command == SetLoc) {
        m_location.setM(m_paramFifo.front()); m_paramFifo.pop();
        m_location.setS(m_paramFifo.front()); m_paramFifo.pop();
        m_location.setF(m_paramFifo.front()); m_paramFifo.pop();
        m_setlocLSN = m_location.toLSN() * 2352;
        m_updateLoc = true;
        m_ints.emplace(InterruptCause::INT3);
        m_responseFifo.push(m_statusCode.r);
        scheduleInterrupt(120000);
        log<Log::CDROM>("SetLoc to {}:{}:{} to LSN: {}\n", m_location.min, m_location.sec, m_location.sect,
                        m_setlocLSN);
    }
    ...
    if (m_command == ReadN) {
        ...
        if (m_updateLoc) {
            ...
            m_discImage.seekg(m_lsn, std::ios::beg);
            m_updateLoc = false;
            log<Log::CDROM>("Implicit Seek to LSN: {}\n", m_lsn);
        }
        scheduleRead();
    }
...
u8 CDROM::read(u32 offset) {
    switch (offset) {
        ...
        default:
            Log::warn("CDROM::read() invalid offset: {}\n", offset);
            return 0;
    }
}
...
void CDROM::write(u32 offset, u8 value) {
    switch (offset) {
        ...
        default:
            Log::warn("CDROM::write() invalid offset\n");
            return;
    }
}
...
void CDROM::write2(u8 value) {
    ...
    switch (m_status.Index.Value()) {
        case 0:
            m_paramFifo.push(value);
            break;
        case 1:
            m_irqEnable = value & 0x1F;
            break;
        case 2:
        case 3:
            break;
...
void CDROM::write3(u8 value) {
    ...
    switch (m_status.Index.Value()) {
        case 0: {
            m_request.r = value;
            if (value & 0x80) {
                log<Log::CDROM>("Request Register: Data Requested\n");
                ...
                log<Log::CDROM>("Request Register: Data Not Requested - Clearing data Fifo\n");
                m_dataFifo.clear();
                m_dataFifoIndex = 0;
                m_status.DataFifoReadReady = 0;
            }
        ...
        case 1: {
            log<Log::CDROM>("Acking IRQ Flags with {:#02x} - previous: {:#02x} new: {:#02x}\n", value & 0x1F,
                            m_irqFlags & 0x1F, m_irqFlags & ~(value & 0x1F));
